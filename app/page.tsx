...
function getYoutubeThumbnail(item: any) {
  return (
...
function isYoutubeLiveItem(item: any) {
...
async function getYoutubeLiveForce() {
  const rows = await safeQuery(`
    SELECT live_status, live_force
    FROM site_settings
    LIMIT 1
  `);
...
async function saveYoutubeAutoLiveStatus(status: "on" | "off") {
  try {
    await db.query(
...
async function getYoutubeVideoDetail(apiKey: string, videoId: string) {
...
async function getCurrentYoutubeLiveVideo(apiKey: string, channelId: string) {
  const liveSearchData = await fetchYoutubeJson(
  ...
  for (const liveItem of liveItems) {
    ...
    const liveDetail = await fetchYoutubeJson(
    ...
    if (item && isYoutubeLiveItem(item)) {
      return {
...
async function getYoutubeVideo() {
  ...
  const forceState = await getYoutubeLiveForce();
  ...
  try {
    const shorts = await getRecentYoutubeShorts(apiKey, uploadsPlaylistId);
    ...
    await saveYoutubeAutoLiveStatus("off");
    return {
      isLive: false,
      title: shorts[0]?.title || "표시할 쇼츠가 없습니다.",
      videos: shorts,
    ...
    console.error("youtube main fetch failed", error);
    return {
      isLive: false,
...
export default async function Home() {
  ...
  <p className="text-xs text-zinc-500">게임하기</p>
          ...
          <aside id="ranking" className="min-w-0 space-y-5">
