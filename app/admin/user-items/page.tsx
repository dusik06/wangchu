            ...
            <tbody>
              {items.map((item: any) => (
                <tr key={item.id} className="border-t border-white/10">
                  <td className="p-4">
                    <p className="font-black">{item.nickname || "닉네임 없음"}</p>
                  ...
                  <td className="p-4">
                    {item.item_image ? (
                      <img
                        src={item.item_image}
                        alt={item.item_name}
                        className="h-16 w-16 rounded-xl object-contain bg-black/30"
                      />
                    ) : (
...
D:\wangchu\frontend\app\admin\user-items\delete-item-button.tsx
